...
function _dispatch() {
  ...
  while (_running < 5 && _q.length) {
    ...
    fetch(url, { signal: AbortSignal.timeout(10000) })
      .then(async res => {
        if (res.status === 429) {
          if (_q.length < 8) setTimeout(() => { _q.unshift({ url, fallback, resolve }); _dispatch() }, 2000)
          else resolve([])
          return
        }
        if (!res.ok) {
          if (fallback) { const r2 = await fetch(fallback, { signal: AbortSignal.timeout(8000) }).catch(() => null); resolve(r2?.ok ? await r2.json().catch(() => []) : []); return }
          resolve([]); return
        }
      ...
      .catch(async () => {
        if (fallback) { const r2 = await fetch(fallback, { signal: AbortSignal.timeout(8000) }).catch(() => null); resolve(r2?.ok ? await r2.json().catch(() => []) : []) }
        else resolve([])
      })
