...
const PROXY = (() => { try { const u = import.meta.env.VITE_SUPABASE_URL; return u ? `${u}/functions/v1/openf1-proxy/v1` : null } catch { return null } })()
...
async function get(endpoint, params = {}) {
  // Use proxy if available (handles auth + CORS), direct URL as fallback
  const baseUrl = PROXY ?? BASE
  const url = new URL(`${baseUrl}${endpoint}`)
  Object.entries(params).forEach(([k, v]) => { if (v != null && v !== '') url.searchParams.set(k, String(v)) })
  const headers = {}
  // Only add token for direct calls (proxy handles auth server-side)
  if (!PROXY) {
...
async function getCached(endpoint, params, ttl = CACHE_MS) {
  ...
  if (data?.length) cache.set(key, { data, exp: Date.now() + ttl })  // never cache empty
  return data
}
...
export const getOvertakes = (sk = 'latest') => getCached('/overtakes', { session_key: sk }, 30_000)
...
export async function buildHistoricalStandings(session_key) {
  try {
    ...
    return result.sort((a, b) => (a.position||99) - (b.position||99)).map((r, i) => {
      ...
      return {
        position: r.position ?? i+1, driver_number: r.driver_number,
        name_acronym: drv.name_acronym ?? r.name_acronym ?? `#${r.driver_number}`,
        full_name: drv.full_name ?? '', team_name: drv.team_name ?? r.team_name ?? '',
        team_colour: drv.team_colour ?? r.team_colour ?? '555555', headshot_url: drv.headshot_url ?? null,
        tyre: stint?.compound ?? null, tyre_age: null, pit_stops: r.number_of_pit_stops ?? 0,
        last_lap: null, best_lap: r.time ?? null, is_personal_best: false,
        is_overall_best: r.position === 1, lap_number: r.number_of_laps ?? 0,
        gap_to_leader: r.gap_to_leader ?? null, interval: null, is_pit_out_lap: false,
        sectors: [], best_sectors: [null,null,null], segments: [], all_laps: [],
      }
    })
  } catch { return [] }
}
